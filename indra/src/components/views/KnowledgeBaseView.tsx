    ...
    <div className="flex-1 min-h-0 h-full flex flex-col bg-[#f8fafc] text-slate-800 overflow-hidden p-6">
      ...
      <div className="flex items-center justify-between pb-4 border-b border-slate-200/80 mb-6">
        ...
        <button
          ...
          disabled={loading}
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white border border-slate-200 hover:border-slate-300 text-xs text-slate-700 hover:text-slate-900 transition-colors font-mono cursor-pointer shadow-2xs"
        >
      ...
      <div className="flex-1 overflow-y-auto space-y-6 scrollbar-thin pr-1">
        ...
        <div
          ...
          className={`border-2 border-dashed rounded-2xl p-6 text-center cursor-pointer transition-all ${
            dragActive 
              ...
              : 'border-violet-200/80 bg-white hover:bg-violet-50/30 hover:border-violet-400 shadow-2xs'
          }`}
        >
          <input
            ref={fileInputRef}
          ...
          <div className="flex flex-col items-center">
            {uploading ? (
              ...
              <>
                <div className="w-10 h-10 rounded-2xl bg-violet-100 flex items-center justify-center mb-2 text-violet-600">
          ...
          <form onSubmit={handleSearch} className="flex gap-2">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-white border border-slate-200 text-xs text-slate-800 placeholder:text-slate-400 outline-none focus:border-violet-400 shadow-2xs transition-colors font-medium"
              />
            </div>
            <button
              type="submit"
            ...
            {searchResults !== null && (
              <button
                ...
                className="px-3 py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-xs text-slate-600 font-medium cursor-pointer"
              >
                Clear
              </button>
            )}
        ...
        <div className="space-y-2">
          ...
          {documents.length === 0 && !loading ? (
            <div className="text-xs text-slate-400 italic p-8 text-center border border-dashed border-slate-200 rounded-2xl bg-white shadow-2xs">
            ...
            <div className="border border-slate-200 rounded-2xl overflow-hidden bg-white shadow-xs">
              <table className="w-full text-left text-xs font-mono">
                <thead className="bg-slate-50/90 text-slate-500 text-[10px] uppercase tracking-wider border-b border-slate-200 font-bold">
                  <tr>
                  ...
                  {documents.map((doc) => {
                    ...
                    return (
                      <tr key={doc.id} className="hover:bg-slate-50/70 transition-colors">
                        <td className="p-3.5 flex items-center gap-2 text-slate-800 font-semibold">
                          {getDocIcon(doc.filename)}
                          <span className="truncate max-w-xs">{doc.filename || doc.name}</span>
                        </td>
                        <td className="p-3.5 text-slate-500">{typeof doc.size === 'number' ? `${(doc.size / 1024).toFixed(1)} KB` : doc.size || '-'}</td>
                        <td className="p-3.5 text-slate-500">{doc.chunk_count || 1}</td>
                        <td className="p-3.5 text-slate-400">{doc.created_at || doc.uploaded_at || 'Recent'}</td>
                        <td className="p-3.5 text-right space-x-2">
                          {isDrawing && (
                            <button
                              ...
                              className="px-2.5 py-1 rounded-lg bg-violet-50 hover:bg-violet-100 text-violet-700 text-[10px] font-bold transition-colors cursor-pointer border border-violet-200/80"
